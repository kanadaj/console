...
export const CAPACITY_BREAKDOWN_QUERIES = {
  ...
  [StorageDashboardQuery.PODS_TOTAL_USED]:
    'sum((kubelet_volume_stats_used_bytes * on (namespace,persistentvolumeclaim,node) group_right() ((kube_running_pod_ready*0+1) * on(namespace, pod)  group_right() (kube_pod_spec_volumes_persistentvolumeclaims_info * on(namespace, pod) group_right(persistentvolumeclaim) kube_pod_info))) * on(namespace,persistentvolumeclaim) group_left(provisioner) (kube_persistentvolumeclaim_info * on (storageclass)  group_left(provisioner) kube_storageclass_info {provisioner=~"(.*rbd.csi.ceph.com)|(.*cephfs.csi.ceph.com)|(ceph.rook.io/block)"}))',
  [StorageDashboardQuery.PODS_BY_USED]:
    '(kubelet_volume_stats_used_bytes * on (namespace,persistentvolumeclaim,node) group_right() ((kube_running_pod_ready*0+1) * on(namespace, pod)  group_right() (kube_pod_spec_volumes_persistentvolumeclaims_info * on(namespace, pod) group_right(persistentvolumeclaim) kube_pod_info))) * on(namespace,persistentvolumeclaim) group_left(provisioner) (kube_persistentvolumeclaim_info * on (storageclass)  group_left(provisioner) kube_storageclass_info {provisioner=~"(.*rbd.csi.ceph.com)|(.*cephfs.csi.ceph.com)|(ceph.rook.io/block)"})',
  [StorageDashboardQuery.CEPH_CAPACITY_TOTAL]: 'ceph_cluster_total_bytes',
  [StorageDashboardQuery.CEPH_CAPACITY_USED]:
    'sum(kubelet_volume_stats_used_bytes * on (namespace,persistentvolumeclaim) group_left(storageclass, provisioner) (kube_persistentvolumeclaim_info * on (storageclass)  group_left(provisioner) kube_storageclass_info {provisioner=~"(.*rbd.csi.ceph.com)|(.*cephfs.csi.ceph.com)"}))',
